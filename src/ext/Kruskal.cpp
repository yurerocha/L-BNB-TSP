#include "Kruskal.h"

Kruskal::Kruskal(const vvi& dist){
	for(uint i = FIRST_NODE; i < dist.size(); ++i){
		for(uint j = FIRST_NODE; j < dist[i].size(); ++j){
			graph.push( make_pair(-dist[i][j], make_pair(i, j)) );
		}	
	}
}

void Kruskal::initDisjoint(int n){
	pset.resize(n);
	for (int i = FIRST_NODE; i < n; ++i){
		pset[i] = i;
	}
}

int Kruskal::findSet(int i){
	return (pset[i] == i) ? i : (pset[i] = findSet(pset[i]));
}

void Kruskal::unionSet(int i, int j){
	pset[findSet(i)] = findSet(j);
}

bool Kruskal::isSameSet(int i, int j){
	return (findSet(i) == findSet(j))? true:false;
}

vii Kruskal::getEdges(){
	return edges;
}

void Kruskal::printEdges(){
	for(uint i = 0; i < edges.size(); ++i){
		cout << edges[i].first << " " << edges[i].second << endl;
	}
}

double Kruskal::MST(int nodes){
	initDisjoint(nodes);

	double cost = 0;

	while(!graph.empty()){
		pair<double, ii> p = graph.top();
		graph.pop();

		if(!isSameSet(p.second.first, p.second.second)){
			edges.push_back(make_pair(p.second.first, p.second.second));
			cost += (-p.first);
			unionSet(p.second.first, p.second.second);
		}
	}

	return cost;
}

double Kruskal::oneTree(int nodes, const vvi& dist){
	double cost = MST(nodes);

	double smallest = INFINITE, secSmallest = INFINITE;
	int i = 1, sec_i = 2;
	for(uint j = 1; j < dist[0].size(); ++j){
		double nb = dist[0][j];
        if(isl(nb, smallest)) {
            secSmallest = smallest;
            smallest = nb;
			sec_i = i;
			i = j;
        } else if (isl(nb, secSmallest) && !iseq(nb, smallest)) {
            secSmallest = nb;
			sec_i = j;
        }
	}

	cost += smallest + secSmallest;
	edges.push_back(make_pair(0, i));
	edges.push_back(make_pair(0, sec_i));

	return cost;
}